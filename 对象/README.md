...
当指定第二个参数的配置属性时，配置属性中**未指定的特性**默认为`false`
...
+ `enumerable`(可枚举)：表明是否可以通过for/in循环返回该属性
...
传入该方法的属性描述符对象不必包含所有4个特性。对于新创建的属性来说，未指定的特性默认的值是`false`或`undefined`。对于修改的已有属性来说，默认的特性值没有做任何修改。*这个方法不能修改继承属性*。
...
`Object.preventExtensions(obj)`来使对象变得*不可扩展*，一但转换就不能在转换回来。（不能在该对象上添加新属性，但自有属性可以修改和删除，该对象的原型还是可以添加属性）。可以通过`Object.preventExtensions(obj)`检测
...
`Object.freeze(obj)`将对象设置为不可扩展和其属性设置为*不可配置且可写特性设置为`false`*，将自有的所有数据属性设置为只读。设置后，对象不可配置不可改值，不可添加值，不可删除属性，但可以通过set函数设置访问器值。可以通过`Object.isFrozen(obj)`检测
```js
//使用该方法后的对象的属性
Ojbect = {
  value: 1,
...
##### in运算符
`in`运算符左侧是属性名（字符串），右侧是对象。如果对象的自有属性或继承属性中包含这个属性（`enmuerable`特性为`true`）则返回`true`
...
+ o中属性p是只读的：不能给只读属性重新赋值（`Object.defineProperty()`方法中   有一个例外，可以对属性特性为`configurable`(可配置)的只读属性重新赋值）
  ```js
  var obj = {
    a: 1,
    b: 2
...
+ o中的属性p是继承属性，且它是只读的：不能通过同名自有属性覆盖只读的继承属性。
  ...
  ```
+ o中不存在自有属性p：o没有使用`setter`方法继承属性p，并且o的可扩展性是`false`;   如果o中不存在p，而且没有`setter`方法可供调用，则p一定会添加至o中;但如果o是不可扩展的，那么o中不能定义新属性。
...
要检测一个对象是否是另一个对象的原型（或处于原型链中）可以使用`obj1.isPrototypeOf(obj2)`方法，例如检测p是否为q的原型：
```js
p.isPrototypeOf(q);
```
